<script lang="ts">
	...
	// Services
	import { mediaTagService, mediaItemService, mediaLibraryStateService } from '$lib/admin/services/media';
	...
	// Spotlight — opens curator at clicked item
	function handleItemClick(item: MediaItem, _event: MouseEvent) {
		// Use 'all' mode so user can browse all items, starting at the clicked one
		curatorMode = 'all';
		curator.syncItems(filteredItems);
		curator.syncTags(tags);
		const index = filteredItems.findIndex((i) => i.id === item.id);
		curator.open(index >= 0 ? index : 0);
	}

	let curatorMode = $state<'all' | 'needsReview'>('needsReview');
...
<div class="media-page">
	...
	{:else}
		<div class="media-layout">
			<!-- Left: Grid + Toolbar (main content) -->
			<div class="main-content">
				<div class="toolbar-row">
					...
					/>
					<button class="filter-toggle" class:active={sidebarOpen} onclick={() => sidebarOpen = !sidebarOpen} title="Toggle tag filters">
						<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5">
							<path d="M2 3h12M4 7h8M6 11h4" stroke-linecap="round"/>
						</svg>
						{#if libraryState.activeTags.length > 0}
							<span class="filter-badge">{libraryState.activeTags.length}</span>
						{/if}
					</button>
			...
			<!-- Right: Tag filter sidebar OR Tag picker panel (for bulk ops) -->
...
<style>
	...
	/* Override the global .sidebar fixed positioning from @austencloud/sidebar package.
	   The TagSidebar component renders a <nav class="sidebar"> which inherits
	   position:fixed from the shared Sidebar.css. We need it in flow here. */
	.right-panel :global(.sidebar) {
		position: static;
		width: 100%;
		height: 100%;
		z-index: auto;
