<script lang="ts">
	...
	async function loadOrders() {
		...
		try {
			...
			orders = all.filter(o => o.status === 'shipped' || o.status === 'delivered');
		} catch (e) {
			console.error('Failed to load fulfilled orders:', e);
		} finally {
			loading = false;
		}
	}
...
<div class="page">
	<div class="page-header">
		<h1 class="page-title">Fulfilled Orders</h1>
	</div>
	{#if loading}
		<div class="loading-rows">{#each Array(4) as _}<div class="skeleton-row"></div>{/each}</div>
	{:else if orders.length === 0}
		<EmptyState title="No fulfilled orders" description="Shipped and delivered orders will appear here" />
	{:else}
		...
		<div class="table-footer">{orders.length} fulfilled order{orders.length !== 1 ? 's' : ''}</div>
	{/if}
</div>
